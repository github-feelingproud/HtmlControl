var ComboTree = function (config) {
    ...
    var template = '<input hidden name="{name}"/><button class="btn btn-default dropdown-toggle" type="button"><label style="display:inline-block; width: calc(100% - 15px)">请选择{label}</label><i class="caret"></i></button><div><div class="combo-tree-search"><div><input class="combo-tree-search-input"/><i class="glyphicon glyphicon-search"></i></div></div><table class="table table-condensed"><tbody class="container"></tbody></table></div>';

    var content = template.replaceAll("{name}", shelf.name).replaceAll("{label}", shelf.label);
    content = $(content).css("width", shelf.width);
    var treeTable = content.find("table").css("width", shelf.width - 20);
